package net.totaldarkness.ChestHistory.client.util.color;
 ...
 */
public abstract class Color implements Colors {
    ...
    // prob this could be made a bit more abstract tbf
    //

    /**
     * Smooth grading from green to red or viceversa
     *
     * @param maxValue    max value, transition happens below it
     * @param actualValue the value that changes and determines the color
     * @param yellow      more nice to look at mode, smooth transition from green to red going through yellow
     * @param reverse     grading from red to green instead of normal one
     */
    public static Color fromGreenToRed(double maxValue, double actualValue, boolean yellow, boolean reverse) {
        if (maxValue == 0) { // avoid dividing by 0
            return WHITE;
        }
        ...
        if (actualValue < maxValue) {
            if (!yellow) { // Goes red -> orange-ish -> red
                green = (int) ((actualValue / maxValue) * 256);
                red = 256 - green;

                if (reverse) {
                    colorIn = Color.of(green, red, 0, 255); // INVERTED
                } else colorIn = Color.of(red, green, 0, 255);
            } else {  // GREEN to YELLOW
                double halfMaxValue = maxValue / 2;

                red = 256 - (int) ((actualValue / maxValue) * 514); // 514 else if lower it overflows turning into green again
                green = (int) ((actualValue / halfMaxValue) * 256) - 256;

                if (reverse) {
                    colorIn = Color.of(255, red, 0, 255); // INVERTED
                } else colorIn = Color.of(red, 255, 0, 255);

                if (actualValue < halfMaxValue) { // YELLOW to RED
                    if (reverse) {
                        colorIn = Color.of(green, 255, 0, 255); // INVERTED
                    } else colorIn = Color.of(255, green, 0, 255);
                }
            }
        } else if (reverse) {
            colorIn = RED;
    ...
    /**
     * Does only regular transition from green to red non reversed (and going through yellow)
     */
    public static Color fromGreenToRed(double maxValue, double actualValue) {
        return fromGreenToRed(maxValue, actualValue, true, false);
    }

    /**
     * Transition goes through yellow, can decide if reversed or not
     */
    public static Color fromGreenToRed(double maxValue, double actualValue, boolean reverse) {
        return fromGreenToRed(maxValue, actualValue, true, reverse);
    }
